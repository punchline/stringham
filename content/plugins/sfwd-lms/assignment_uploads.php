...
add_action("parse_request", "learndash_assignment_process_init");
			...
			if(!empty($assignment_data)) {
				...
				foreach($assignment_data as $k=>$v)
				{
					...
					if($assignment_post_id) {
						$assignment_posts_ids[] = $assignment_posts_id;
						foreach ($assignment_meta as $key => $value) {
							update_post_meta($assignment_post_id, $key, $value);
						}
						if(learndash_is_assignment_approved($assignment_posts_id) === true)
							learndash_approve_assignment_by_id($assignment_posts_id);
					}
					else {
						$error = true;
...
function learndash_get_assignments_list($post) {
	$posts = get_posts("post_type=sfwd-assignment&page_per_posts=-1");
	if(!empty($posts))
...
function learndash_restrict_assignment_listings($query) {
	...
	if( is_admin() AND $pagenow == 'edit.php' AND $query->query['post_type'] == 'sfwd-assignment')  {
...
$labels = array(
    'name' => _x('Assignments', 'learndash'),
    'singular_name' => _x('Assignment', 'learndash'),
    'edit_item' => _x('Edit Assignment', 'learndash'),
    'view_item' => _x('View Assignment', 'learndash'),
    'search_items' => _x('Search Assignments', 'learndash'),
    'not_found' => _x('No assignment found', 'learndash'),
    'not_found_in_trash' => _x('No assignment found in Trash', 'learndash'),
    'parent_item_colon' => _x('Parent:', 'learndash'),
    'menu_name' => _x('Assignments', 'learndash'),
);
