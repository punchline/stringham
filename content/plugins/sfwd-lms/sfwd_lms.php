...
if ( !class_exists( 'SFWD_LMS' ) ) {
	class SFWD_LMS extends Semper_Fi_Module  {
		...
		function add_post_types() {
			...
			$post_args = Array(
					...
					//  'taxonomies' => Array( 'courses' => __('Manage Course Associations', 'learndash') ),
					  'cpt_options' => Array( 'hierarchical' => 'false', 'supports' => array ( 'title', 'editor', 'thumbnail' , 'author', 'comments', 'revisions') , 'labels' => $course_labels),
					...
					Array(
					  ...
					  'template_redirect' => true,
					//  'taxonomies' => Array( 'courses' => __('Manage Course Associations', 'learndash') ),
					  'cpt_options' => Array( 'has_archive' => false, 'supports' => array ( 'title', 'thumbnail', 'editor', 'page-attributes' , 'author', 'comments', 'revisions'), 'labels' => $lesson_labels ),
					  'fields' => Array(
					...
					Array(
					  ...
					  'template_redirect' => true,
					//  'taxonomies' => Array( 'courses' => __('Manage Course Associations', 'learndash') ),
					  'cpt_options' => Array( 'has_archive' => false, 'hierarchical' => false, 'supports' => array ( 'title', 'thumbnail', 'editor' , 'author', 'page-attributes' ,'comments', 'revisions' ) , 'labels' => $quiz_labels),
					  'fields' => 
					  Array( 
					...
					Array(
					  ...
					  'template_redirect' => true,
					//  'taxonomies' => Array( 'courses' => __('Manage Course Associations', 'learndash') ),
					  'cpt_options' => Array( 'supports' => array ( 'title', 'thumbnail', 'editor', 'page-attributes' , 'author', 'comments', 'revisions'),  'show_in_nav_menus' => false , 'show_in_menu'	=> false, 'has_archive' => false, 'labels' => $lesson_topic_labels),
					  'fields' => Array(
		...
		function select_a_course($current_post_type = null) {
			...
			$posts = ld_course_list(array('array' => true, 'post_status' => 'any',  'orderby' => 'title', 'order' => 'ASC'));
		...
		*/
			...
			if ( !empty( $settings["{$courses_prefix}course_prerequisite"] ) ) {
					$posts = get_posts( Array( 'post_type' => 'sfwd-courses'  , 'post_status' => 'any',  'numberposts' => -1) );
					$post_array = Array( '0' => __('-- Select a Course --', 'learndash') );
					if ( !empty( $posts ) )
						foreach( $posts as $p )
							if($p->ID == get_the_id()){ 
...
function nss_plugin_updater_activate_sfwd_lms()
{
	...
	function learndash_menu() {
		if(!is_admin())
			return;
	    add_menu_page(__("LearnDash LMS", "learndash"), __("LearnDash LMS", "learndash"), "manage_options", "learndash-lms", null, null, null);
	    ...
	    add_submenu_page("learndash-lms-non-existant", __("Course Shortcodes", "learndash"), __("Course Shortcodes", "learndash"), "manage_options", "learndash-lms-course_shortcodes", "learndash_course_shortcodes_page");
