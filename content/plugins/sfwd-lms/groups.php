...
function learndash_group_emails(){
	...
	function learndash_groups_post_content() {
		$labels = array(
			'name'               => _x( 'LearnDash Groups', 'post type general name' ),
			'singular_name'      => _x( 'LearnDash Group', 'post type singular name' ),
			'add_new'            => _x( 'Add New', 'learndash' ),
...
function learndash_groups_page_box( $post ) {
	...
	$all_group_leaders = get_users("role=group_leader&orderby=display_name&order=ASC");
...
function learndash_group_admin_menu() {
	add_submenu_page("edit.php?post_type=sfwd-courses", __("Group Administration","learndash"), __("Group Administration","learndash") ,'group_leader','group_admin_page', 'learndash_group_admin_menu_page');
}
...
function is_group_leader($user_id) {
	$user = get_user_by("id", $user_id);
	if(empty($user->roles) || !is_array($user->roles))
	return false;
	else
	return in_array("group_leader", $user->roles);
}
