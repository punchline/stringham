...
class learndash_woocommerce {
	...
	function __construct() {
		...
		add_action('woocommerce_order_status_completed', array($this, 'send_receipt'), 1, 3);
		add_action('woocommerce_order_status_processing', array($this, 'send_receipt'), 1, 3);
	}
	...
	function add_scripts(){
		wp_enqueue_script( 'ld_wc', plugins_url('/learndash_woocommerce.js', __FILE__) );
	}

	function add_front_scripts(){
		wp_enqueue_script( 'ld_wc_front', plugins_url('/front.js', __FILE__) );
	}
	...
	function send_receipt($order_id, $status, $new_status){
