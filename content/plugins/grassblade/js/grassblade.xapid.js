var grassblade_lrs = GrassBlade.GetLRSObject();


(function( window, undefined ) {
"use strict";
window.GrassBlade = this;
...
function GetLRSObject(){
    ...
    if(lrs.endpoint === undefined || lrs.endpoint == "" || lrs.auth === undefined || lrs.auth == ""){
        Log("Configuring TCDriver LRS Object from queryString failed");
        return null;
    }
...
function _PrepareStatement(lrs, stmt) {
...
// Synchronous if callback is not provided (not recommended)
function SendStatement (lrs, stmt, callback) {
    if (lrs.endpoint != undefined && lrs.endpoint != "" && lrs.auth != undefined && lrs.auth != ""){
		_PrepareStatement(lrs, stmt);
        XHR_request(lrs, lrs.endpoint+"TCAPI/statements/?statementId="+_ruuid(), "PUT", JSON.stringify(stmt), lrs.auth, callback);
    }
}

// Synchronous if callback is not provided (not recommended)
function SendMultiStatements (lrs, stmtArray, callback) {
    if (lrs.endpoint != undefined && lrs.endpoint != "" && lrs.auth != undefined && lrs.auth != ""){
...
// Synchronous if callback is not provided (not recommended)
function SetState (lrs, activityId, stateKey, stateVal, callback) {
    if (lrs.endpoint != undefined && lrs.endpoint != "" && lrs.auth != undefined && lrs.auth != ""){
...
// Synchronous if callback is not provided (not recommended)
function GetState (lrs, activityId, stateKey, callback) {
    if (lrs.endpoint != undefined && lrs.endpoint != "" && lrs.auth != undefined && lrs.auth != ""){
        var url = lrs.endpoint + "TCAPI/activities/state?activityId=<activity ID>&actor=<actor>&stateId=<statekey>";
...
// Synchronous if callback is not provided (not recommended)
function SendActivityProfile (lrs, activityId, profileKey, profileStr, lastSha1Hash, callback) {
...
// Synchronous if callback is not provided (not recommended)
function GetActivityProfile (lrs, activityId, profileKey, callback) {
...
// Synchronous if callback is not provided (not recommended)
function GetStatements (lrs,sendActor,verb,activityId, callback) {
    if (lrs.endpoint != undefined && lrs.endpoint != "" && lrs.auth != undefined && lrs.auth != ""){
...
TCStatement.prototype.Send = function() {
    SendStatement(this.lrs,this.GetStatement());
}
...
DateFromISOString = function(string) {
