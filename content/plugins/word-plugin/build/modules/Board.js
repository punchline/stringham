...
function(Selector, Grid, $, eventBus, UI) {
    ...
    function Board(options) {
        ...
        $.on(window, "resize", _.bind(this.redraw, this));
    }
    ...
    proto.calcSizes = function() {
        this.options.fontSize = this._originalFontSize || this.options.fontSize;
        this.cellSize  = ~~(this.options.fontSize * CELL_RATIO);
        this.boardSize = this.cellSize * this.options.size;
        var containerSize = (this.options.oldContainer.offsetWidth );
        
        if (containerSize < this.boardSize) {
            this._originalFontSize = this.options.fontSize;
            this.options.fontSize = ~~(((containerSize)/ this.options.size) / CELL_RATIO);
            this.cellSize  = ~~(this.options.fontSize * CELL_RATIO);
            this.boardSize = this.cellSize * this.options.size;
        }
    ...
    proto.draw = function() {
        var grid      = this.data.grid,
            ...
            offsetX   = parseInt(fontSize * OFFSET_RATIO),
            offsetY   = parseInt(fontSize * CELL_RATIO * OFFSET_RATIO, 10);
