...
function(UI, Board, Timer, Description, eventBus, $, Downloader) {
    ...
    function Soup(options) {
        ...
        options.uid = (new Date).getTime() + _.uniqueId();
        ...
        $.getData(this.options.oldContainer.previousSibling, this.channel);
    }
    ...
    function initEvents(self) {
        $.on($.$("show-help-"+self.options.uid), "click", function(e){
            e.preventDefault();
            self.ui.Modal.open("help");
        });

        $.on($.$("restart-"+self.options.uid), "click", function(e){
            e.preventDefault();
            self.restart();
        });
